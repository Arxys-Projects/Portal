...
// One quote-document download button (ADR 0083 / 0089). The href points at the
// shared route; pricing appears only inside the rendered PDF, never as UI here.
function QuoteDocLink({ href, label }: { href: string; label: string }) {
  return (
    <a
      href={href}
      download
        ...
        <div className="mt-6 space-y-6">
          {groups.map((group) => (
            ...
            >
              ...
              {/* Quote documents (ADR 0083 + 0089) — the partner's own quote
                  revisions. Each revision offers two documents: the Project
                  Quote (partner pricing) and the Customer Proposal (end-user
                  version, no discount). Both appear only once a snapshot exists;
                  pricing lives only inside the rendered PDFs. */}
              {group.quotes && group.quotes.length > 0 ? (
                <div className="border-b border-line-soft bg-panel px-4 py-2.5">
                    ...
                    {group.quotes.map((q) => (
                      ...
                      >
                        <span className="text-xs font-semibold text-ink-soft">
                          v{q.version} · {formatDate(q.createdAt)}
                        </span>
                        <QuoteDocLink
                        ...
                        />
                        <QuoteDocLink
                          href={`/api/submissions/${q.submissionId}/project-quote/pdf?version=${q.version}&variant=customer-proposal`}
