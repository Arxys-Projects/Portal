...
- **Same-day follow-up — §7.6 actioned live** (audit §8 addendum): re-audited the replacement Milestone Solution Designer through Andy's authenticated Chrome session (Andy driving the UI, the audit reading the design's server-rendered state + client code from a second tab). Four of five anchor values re-confirmed exactly (1966/4424/6637/9832); decimal Kbit confirmed in live conversion code; Milestone's own fps scaling measured at exponent ≈0.90 over a 10/12/15/18 fps sweep (not linear); H.264:H.265 ratio 1.724 vs engine's inherited 1.714; motion confirmed as recording duty-cycle (data rate invariant to motion %); capacity margin is an explicit Max. Disk Utilization slider defaulting 70–90% — Arxys's stacked ×1.44 matches Milestone's most conservative default (×1.43). New caveat: MSD's "4MP" bucket is now 2592×1520, not 2560×1440.
...
- **Fourth batch: the Security Workstations hero**, landed as `sw-front.png` — one shot for SW10
  and SW20, named for `families.ts`'s `slug: "sw"` and the Price Book's existing `sw-hero.png`
  rather than a model list.
  - **RETRACTED, and worth reading as a lesson: this asset is not off-size.** It was first
    recorded here as "the first off-size asset, 557×110 where every other hero is 720×240", with
    a recommendation to re-export at 720×240. That was wrong, and the error was mine, not the
    artwork's. The design handoff in `datasheets/design_handoff_videox_datasheet/README.md`
    defines **two** templates: "Ledger" for the V100–V800 NVRs and V250/V255 (720×240 hero) and
    **"Rail" for the SW workstations — a single page with a 214px left rail and a photo slot of
    513×110 that bleeds right by the 44px content padding**. Render width is therefore
    `816 − 214 − 1 − 44 − 44 = 513`, plus the 44px bleed = **557**. The supplied image is
    557×110: pixel-exact for the slot it was drawn for.
  - **The real defect was evaluating an asset against the wrong template.** It letterboxed and
    looked unfinished because it was rendered in the Ledger 720×240 frame, which is not its
    frame. Nothing about the file needs changing. Had the recommendation gone out, a correct
    asset would have been re-cropped to fit the wrong layout.
  - Standing lesson: before judging a photo's dimensions, establish which template it belongs to.
    The handoff README is the authority, and it was in the repo the whole time.
...
### Data remediation applied (production)

- **Lineage backfill: 7 writes**, each guarded on `parent_submission_id is null` so a re-run cannot clobber, and each verified beforehand to have a strictly-older parent. Applied to the one provable chain (Status Automation, 3 rows sharing deal #5415) and the 6 signature-matching pairs (Mineola, Foothills, Huntington Beach, Galloway, North Bergen). Post-check: 8 rows carry lineage, no self-references, no cycles, no dangling parents. The 5 accidental double-submit clusters and the Dallas 5-row chain were deliberately left alone.
- **The relink fix is confirmed working end-to-end** on the `test` submission: its source deal #5367 really was soft-deleted, Retry created #5439, no duplicate.
- **The sequencing warning turned out to be load-bearing.** Two Retry clicks landed *before* the backfill, so North Bergen and Galloway each got a fresh deal (#5441, #5440) alongside their original (#5438, #5430) — two open deals per job. Notably the auto-created ones were the *more* correct: right on-behalf company (JCT Solutions, not "Arxys") and values matching the portal, whereas the originals carry hand-edits made in Pipedrive. Stakeholder chose to keep the originals for their sales history; the two revision rows were repointed to #5438/#5430 (guarded on the duplicate id, so idempotent), leaving each pair sharing one deal — the same shape a natively-successful revise produces. Deleting the redundant #5440/#5441 is left to the CRM owner.
- **Still unlinked: 5 submissions.** Mineola, Huntington Beach and Foothills now have lineage, so Retry will inherit their parent's deal rather than duplicate. BCUA and the two Dallas rows have no lineage; Dallas must not be retried before its chain is decided (3 deals across 5 rows already). WCJ (`4e647f4f`) has no list price and `buildRelinkInputs` will refuse it by design.
...
- 264/264 tests pass (6 new), `tsc --noEmit` clean. Merged directly to `main` via [PR #8](https://github.com/Arxys-Projects/Portal/pull/8) at the user's explicit direction — no feature branch left dangling, closing the exact commit-but-not-merged gap that caused the 3rd recurrence.
- **Applied the migration to production** (`supabase db push`) the same session. See [ADR 0093](./decisions/0093-submission-duplicate-lineage-and-relink.md)'s "Correction" and "Step 2 status" sections for full detail, and the detour below for a real process risk this surfaced.
...
- **`supabase migration list` showed 6 local-only (unapplied-looking) migrations; 3 of them were already live.** `db push` immediately errored on the oldest ("policy already exists") trying to re-run `20260720000001` (ADR 0083's project_quotes partner-select policy). Root cause: this repo's own convention (ADR 0083, ADR 0089) requires security- or schema-sensitive migrations to be applied by hand via the Supabase dashboard SQL editor rather than CLI `db push` — each such file is marked with a "STOP-AND-FLAG" comment saying so — and a dashboard-applied change never updates the CLI's migration-history tracking table. Verified independently (not just trusted the comment) via read-only REST calls confirming `partners.logo_path` and the `partner-logos` bucket both already exist in production, then repaired tracking with `supabase migration repair --status applied` for the 3 confirmed-live versions. Two more local-only migrations (`20260723000001`/`20260723000002`, the datasheet `appliance_specs`/`product_specs` columns) were genuinely still pending but belong to a *separate, paused* initiative not yet approved for production (ADR 0090) — since `db push` applies everything pending in one all-or-nothing batch, those two files were temporarily moved out of `supabase/migrations/`, the push ran with only the target migration present, then the files were moved back (confirmed zero working-tree diff afterward). Nothing about this is specific to the North Bergen bug — it's a standing risk any future migration push in this repo needs to check for first.
...
Split "a portal/Excel price becomes effective" from "Pipedrive gets updated", per
brief. Inspected first and reported back before touching anything: the price table is
`public.products` (PK `sku`), the "pipeline" is spread across `validate-prices-sheet.ts`
+ `backup-tables.ts` + `backup-pipedrive-products.ts` + `push-prices.ts` (dry-run + live),
and the portal price book (`price-book/page.tsx`, `[slug]/page.tsx`) and the Excel export
(`api/price-book/xlsx/route.ts`) each queried `products` with their **own** query (no
shared resolver). Confirmed **no FK references `products(sku)`**, so dropping the PK is safe.

- **Migration (applied 2026-07-02):** [`20260702000001_price_versioning_append_only.sql`](../supabase/migrations/20260702000001_price_versioning_append_only.sql).
  `products` → append-only: surrogate `id` PK (`generated by default`), `effective_date`
  (backfilled to `2026-05-05`, the published "Effective" date), `pushed_to_pipedrive_at`,
  `unique (sku, effective_date)`, and a `current_products` view (`security_invoker = on`)
  resolving `distinct on (sku) … where effective_date <= current_date`. Rollback:
  [`price-versioning-append-only-rollback.sql`](../supabase/rollback/price-versioning-append-only-rollback.sql).
- **Read side:** all 12 `products` read sites in `src/` repointed to `current_products`
  (mechanical table-name swap — identical select lists; view is one-row-per-SKU so
  `.maybeSingle()` stays valid). The view is the single shared resolver the portal + Excel
  now share. No writes to `products` exist in `src/` (writes are service-role only).
- **Pipeline:** `push-prices.ts` gains `--target=portal|pipedrive|all` (default `all`).
  `portal` inserts versioned rows (`--effective-date`, past/future; never touches Pipedrive
  or the stamp). `pipedrive` pushes current-as-of-today prices (reads `current_products`)
  and stamps `pushed_to_pipedrive_at`; diff-based, so re-runs are no-ops. `all` = portal
  then pipedrive, matching the monthly cycle. Dry-run diff now shows old→new price,
  effective date, and a "Touches Pipedrive: YES/NO" line.
- Changed files typecheck clean (`tsc --noEmit`); remaining tsc errors are pre-existing
  in `.test.ts` files (React-PDF / exceljs type mismatches), unrelated to this change.
- **Shipped (after Andy's approval):** migration applied via `supabase db push` and
  verified against the cloud DB (`current_products` returns exactly one row per SKU,
  `effective_date` backfilled to `2026-05-05`, `pushed_to_pipedrive_at` null, surrogate
  `id` present). Then committed (`23268c6`) and pushed `main` → Vercel prod — migration
  **first**, code second, so product reads never hit a missing view.

### Decisions captured

- [`0076-append-only-price-versioning.md`](./decisions/0076-append-only-price-versioning.md)
