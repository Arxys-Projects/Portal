...
- **Status**: Accepted
- **Date**: 2026-05-28
- **Amended**: 2026-06-11 — three Pipedrive field display names renamed (`Recording` → `Recording New`, `CODEC` → `CODEC New`, `Scene Complexity` → `Complexity Scene-Motion`). Field types, option IDs, and aggregation logic are unchanged; only the display-name strings the integration matches on were updated. See the 2026-06-11 JOURNAL entry.
...
The complication is field *type*. The Pipedrive deal fields are a mix: Frame Rate / Motion Activity % / Resolution / Recording hours are free-text (`varchar`), Complexity Scene-Motion is a multi-select **set**, and Recording New and CODEC New are single-select **enums** that hold exactly one option ID. "List them all" is trivial for free-text and set fields, but structurally impossible for a single-select enum.
...
- **Force CODEC New/Recording New to free text too.** Would let them list all values, but these are admin-curated enums with option taxonomies — writing free text to an enum field is rejected/dropped by Pipedrive and we don't own those field types. Rejected.
...
- **Complexity Scene-Motion (multi-select set):** all distinct tier option IDs, comma-joined and sorted → `"287,289"`. This is the one field type that can genuinely hold the full set.
- **Recording New (single-select enum):** can't list multiple, so it collapses deterministically — if **any** group records below 100% duty cycle the whole deal flips to "On Motion" (119); all-continuous stays "Continuous" (118).
- **CODEC New (single-select enum):** holds one option, so send the **dominant** codec — the one used by the most cameras summed across groups; ties resolve to first-seen. Minority codecs are not shown on this field.
...
**Negative:**
- CODEC New shows only the dominant codec; a mixed-codec project loses the minority codec on that field. Accepted — the field type can't represent more, and the dominant codec is the most representative single value.
...
- Recording New's "any-motion ⇒ On Motion" is lossy: a deal that's mostly continuous with one motion-only group reads as On Motion. Accepted as the conservative, deterministic collapse.
...
- If a Pipedrive admin converts CODEC New or Recording New to a multi-select set, switch them to the same distinct-list treatment as Complexity Scene-Motion.
