...
**Still open from §7.6**: only MSD's end-to-end storage output (per-group TB on the proposal/summary screen) — reverse it against the now-known bitrates to confirm how hours × days × motion% × disk-utilization compose, and whether GB is decimal on the document.
